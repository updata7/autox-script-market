...
android {
    defaultConfig {
        applicationId = "com.cc.scriptstore"
...
//        multiDexEnabled = true
    ...
    buildTypes {
        named("debug") {
            ...
            // 设置 debug 环境的 BASE_URL
            buildConfigField("String", "BASE_URL", "\"http://192.168.31.151:8001\"")
            resValue("string", "default_value_script_dir_path", "/ScriptDebug/")
        }
        named("release") {
            isShrinkResources = false
            isMinifyEnabled = false
            ...
            buildConfigField("String", "BASE_URL", "\"https://xxx.com\"")
            resValue("string", "default_value_script_dir_path", "/Script/")
        }
    }
...
//            buildConfigField("String", "APPID", "\"?id=23\"")
            manifestPlaceholders.putAll(mapOf("appName" to "脚本超市 v6"))
        }
    }
...
val skipDocInstallation = project.findProperty("skipDocs")?.toString()?.toBoolean() ?: false

if (!skipDocInstallation) {
    tasks.named("preBuild").dependsOn("installationDocumentation")
} else {
    logger.lifecycle("Skipping documentation installation as per user request (skipDocs=true)")
}
...
tasks.register("installationDocumentation") {
    ...
    doFirst {
        ...
        try {
            logger.lifecycle("开始下载文档: $docV1Uri")
            okhttp3.OkHttpClient().newCall(Request.Builder().url(docV1Uri).build()).execute()
                .use { response ->
                    if (!response.isSuccessful) {
                        logger.warn("文档下载失败: ${response.code}. 继续构建但不包含文档。")
                        return@doFirst
                    }
            ...
            logger.lifecycle("文档下载安装完成")
        } catch (e: Exception) {
            logger.warn("文档下载或解压过程中发生错误: ${e.message}. 继续构建但不包含文档。")
            // 创建一个占位文件，以便下次构建不再尝试下载
            docV1Dir.mkdirs()
            File(docV1Dir, "download_failed.txt").writeText("Documentation download failed: ${e.message}")
        }
    }
}
tasks.named("clean").configure {
