...
open class ExplorerViewKt : ThemeColorSwipeRefreshLayout, OnRefreshListener,
    ...
    private val TAG = "ExplorerViewKt"
    ...
    private fun initExplorerItemListView() {
        ...
        manager.spanSizeLookup = object : GridLayoutManager.SpanSizeLookup() {
            override fun getSpanSize(position: Int): Int {
                //For directories
                val ret = if (position > positionOfCategoryDir && position < positionOfCategoryFile()) {
                    directorySpanSize1
                } else 2
                //For files and category
                Log.d(TAG, "getSpanSize $ret")
                return ret
            }
        }
    ...
    private fun loadItemList() {
        ...
        explorer!!.fetchChildren(currentPageState.currentPage)
            ...
            .collectInto(explorerItemList.cloneConfig()) { obj: ExplorerItemList, item: ExplorerItem? ->
                Log.d(TAG, "collectInto ExplorerItem ==> $item")
                obj.add(
                    item
                )
            }
    ...
    protected open fun onCreateViewHolder(
        ...
        return when (viewType) {
            VIEW_TYPE_ITEM -> {
                Log.d(LOG_TAG, "script_file ==> $VIEW_TYPE_ITEM")
            ...
            VIEW_TYPE_PAGE -> {
                Log.d(LOG_TAG, "script_directory ==> $VIEW_TYPE_PAGE")
                ...
                Log.d(LOG_TAG, "script_category ==> $viewType")
                MyLog.printTrace()
                CategoryViewHolder(
                    inflater.inflate(
    ...
    private inner class ExplorerAdapter : RecyclerView.Adapter<BindableViewHolder<Any>>() {
        ...
        override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): BindableViewHolder<Any> {
            ...
            MyLog.d(LOG_TAG, "viewType ==> $viewType")
    ...
    inner class ExplorerItemViewHolder internal constructor(itemView: View) :
        ...
        override fun bind(item: Any, position: Int) {
            if (item !is ExplorerItem) {
                MyLog.d("not explorerItem $position")
            ...
            // 不给编辑
            edit.visibility = GONE //if (item.isEditable) VISIBLE else GONE
