...
private val TAG: String? = "Agent_MainActivity"
...
class MainActivity : FragmentActivity() {
    ...
    private val myScriptListFragment by lazy { MyScriptListFragment() }
    private val taskManagerFragment by lazy { TaskManagerFragmentKt() }
    private val mallManagerFragment by lazy { MallManagerFragment() }
...
//                                scriptListFragment.explorerView.onRefresh()
                    ...
                    MainPage(
                        ...
                        myScriptListFragment = myScriptListFragment,
                        mallManagerFragment = mallManagerFragment,
...
fun MainPage(
    ...
    // by 有委托，用mutableState 管理 currentPage
...
private fun getBottomItems(context: Context) = mutableStateListOf(
    ...
    BottomNavigationItem(
        ...
        context.getString(R.string.text_mall)
    ),
//    BottomNavigationItem(
//        R.drawable.ic_manage,
...
private fun NewDirectory(
    ...
    DropdownMenuItem(onClick = {
        Log.d(TAG, "new directory clicked")
        onDismissRequest()
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) {
            if (Environment.isExternalStorageManager()) {
                getScriptOperations(
